#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `ores-otel-api-server`.
//!
...
//! to ores-otel — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `ores-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `ores-otel-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `ores-otel-api-server` cluster |
...
//! `ores-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `ORES_OTEL_API_URL`, `ORES_OTEL_NATS_URL`, and the rest; see the
