...
for file in .ores-mw.toml .ores-otel.toml .ores-rl.toml .ores-lru.toml .shared-auth.toml config/ores-middleware-stack.json conformance/check-runtime-configs.rs; do
...
rustc --edition=2021 --test conformance/check-runtime-configs.rs -o "$tmp.test"
"$tmp.test"
rustc --edition=2021 -O conformance/check-runtime-configs.rs -o "$tmp"
"$tmp"
